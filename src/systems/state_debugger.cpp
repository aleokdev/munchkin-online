...
void StateDebugger::render() {
	ImGui::Begin("Debug");
	{
		...
		if (ImGui::TreeNode("Card Decks")) {
			...
			ImGui::Text("Cards in dungeon discard deck: %zu", game->state.dungeon_discard_deck.size());
			ImGui::Text("Cards in treasure deck: %zu", game->state.treasure_deck.size());
			ImGui::Text("Cards in treasure discard deck: %zu", game->state.treasure_discard_deck.size());
			ImGui::TreePop();
		}
