...
#undef DEFINE_OPERATOR
    ...
    float get_length() const { return std::sqrt(std::pow(x, 2.0f) + std::pow(y, 2.0f)); }
    ...
    static Vec2D lerp(Vec2D const& a, Vec2D const& b, float factor) {
        return Vec2D{(1 - factor) * a.x + factor * b.x, (1 - factor) * a.y + factor * b.y};
    }
...
struct Rect2D {
    ...
    Vec2D get_center() const { return pos + (size / 2); }
};
